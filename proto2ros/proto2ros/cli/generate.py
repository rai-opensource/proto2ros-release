...
def package_defaults() -> Generator[argparse.Namespace, None, None]:
    """Returns package defaults."""
    if sys.version_info >= (3, 9):
        files = importlib.resources.files("proto2ros.configuration")
        managed_path = importlib.resources.as_file(files.joinpath("default.yaml"))
        with managed_path as path:
            yield argparse.Namespace(config_file=path)
    else:
        with importlib.resources.path("proto2ros.configuration", "default.yaml") as path:
            yield argparse.Namespace(config_file=path)


def parse_args(defaults: argparse.Namespace) -> argparse.Namespace:
...
def main() -> int:
    ...
    with package_defaults() as defaults:
